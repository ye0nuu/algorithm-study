...
# ✅ 문제 설명 요약:
# 2차원 평면 위의 점 N개가 주어진다. 좌표를 y좌표가 증가하는 순으로, y좌표가 같으면 x좌표가 증가하는 순서로 정렬한 다음 출력 

# ✅ 내 풀이 아이디어:
# - N을 입력받고, 좌표를 저장하기 위한 coor 배열을 만든다.
# - coor에 좌표를 2차원 배열로 입력받고, sort() 함수로 정렬하는데 y를 우선으로 하여 정렬해야 하므로,
# - key 파라미터에 람다함수를 사용하여 y를 우선으로 보고, 같을 경우 x를 보도록 설정
